...
export default function AboutMe({
  githubUsername = "Gunjankumar55",
  leetcodeUsername = "gunjankumar55",
  theme = "tokyonight",
}) {
...
>
  <p className="text-base md:text-lg leading-relaxed font-bold text-brand-dark">
    ...
    <span className="px-1.5 py-0.5 bg-brand-rose border-2 border-brand-dark inline-block rotate-[-1deg] text-brand-dark">
      Gunjankumar N. Choudhari
    </span>{" "}
    — a{" "}
    <span className="underline decoration-brand-red decoration-3">
      Computer Engineer
    </span>{" "}
    who loves turning ideas into working software.
  </p>

  <div className="mt-4 flex flex-wrap gap-2">
    <span className="inline-block px-2.5 py-1 bg-brand-dark text-brand-bg text-xs font-display uppercase tracking-wider rotate-[-1deg] border-2 border-brand-dark">
      Incoming System Engineer @ TCS
    </span>
  </div>

  <p className="mt-4 text-base md:text-lg leading-relaxed text-brand-dark/80">
    I enjoy building practical software, exploring machine learning and cloud
    technologies and continuously picking up new tools along the way. My
    goal is simple {" "}
    <span className="font-bold text-brand-dark">
      build things that are useful, scalable and make a real difference.
    </span>
  </p>
</NeoWindow>
{/* Education Card */}
<NeoWindow title="Education" icon={GraduationCap} headerBg="bg-brand-rose" textCol="text-brand-dark">
  <div className="font-bold text-brand-dark">
    <span className="text-xl font-display font-black uppercase block leading-snug">
      B.Tech in Computer Engineering
    </span>
    <span className="inline-block mt-1 px-2 py-0.5 bg-brand-dark text-brand-bg text-xs font-display uppercase tracking-wider rotate-[-1deg]">
      Minor: Data Science
    </span>
    <span className="text-brand-red text-lg block mt-2">
      D. Y. Patil Ramrao Adik Institute of Technology
    </span>
    <div className="mt-3 p-3 bg-brand-bg border-2 border-brand-dark neo-shadow-sm flex flex-col gap-1 text-sm md:text-base">
      <span>CGPA: 9.61 / 10</span>
